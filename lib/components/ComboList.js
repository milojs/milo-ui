...
const { doT, check } = util;
const componentsRegistry = registry.components;
const Match = check.Match;

const TOOLTIP_TEMPLATE = doT.compile(readFileSync(`${__dirname}/../forms/items/partials/tooltip.dot`, 'utf-8'));
        ...
        template: `
            ...
            <div ml-bind="MLList:list">
                <div ml-bind="MLListItem:item">
                    ...
                    <div ml-bind="[template]:tooltip"></div>
                </div>
            </div>
        `
    }
...
_.extendProto(MLComboList, {
    ...
    renderToolTips: MLComboList$renderToolTips
});
...
function MLComboList$renderToolTips () {
    const items = this._list.list.getItems();
    for (const item of items) {
        const { tooltip } = item.container.scope;
        const { tooltip:tooltipData } = item.data.get();
        item.dom.toggleCssClasses('has-tooltip', !!tooltipData);
        if(!tooltipData) return;
        tooltip.template.set(TOOLTIP_TEMPLATE);
        tooltip.template.render({ item: tooltipData });
    }
}
