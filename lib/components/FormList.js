...
function MLFormList$init() {
    ...
    // _manageFormValidation.call(this);
...
function MLFormList$validateModel(callback, invalidControls) {
    ...
    _.eachKey(this._dataValidations.fromModel, function (validators, modelPath) {
        ...
        var data = this.model.m().get()[index][path];
    ...
    async.each(validations,
        function (validation, nextValidation) {
            ...
            async.every(validation.validators,
                ...
                function (err, valid) {
                    ...
                    createOnValidatedd.call(self, lastResponse, invalidControls);
        ...
        function (err) {
            invalidControls = {...invalidControls, ...self._invalidFormControls};
...
function createOnValidatedd(response) {
    var reason;
    if (response.valid)
        delete this._invalidFormControls[response.path];
    else {
        reason = {
            label: `List Item ${response.path}`,
