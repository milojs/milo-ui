...
function _processSchemaSubscriptions(comp, subscriptions) {
    _.eachKey(subscriptions, (subscriber, name) => {
        const context = typeof subscriber == 'object' ? subscriber.context : null;
        if (context && context != 'facet' && context != 'owner') {
            subscriber = Object.assign(
                {}, subscriber,
                {context: getFunctionContext.call(this, context)}
            );
        }
        this.model.on(name, subscriber);
    });
}
