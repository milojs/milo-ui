...
function MLSuperCombo$update() {
    ...
    var escapeHTML = document.createElement('div');
    var arrToShow = this._filteredOptionsData.slice(this._startIndex, this._endIndex).map(function(option) {
        escapeHTML.textContent = option.label;

        return {
            value: option.value,
            label: escapeHTML.innerHTML
        };
    });
