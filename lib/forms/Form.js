...
function MLForm$getSubSchemas() {
    console.log("this._formViewPaths", this._formViewPaths);
    debugger
    let subSchemas = [];
...
function processSchema(comp, schema, viewPath, formViewPaths, formModelPaths, modelPathTranslations, dataTranslations, dataValidations) {
    ...
    // if(schema.subSchema && schema.subSchema.items) {
    //     processSubSchema(schema.subSchema.items)
    // }

    if (schema.items )
        _processSchemaItems.call(this, comp, schema.items || schema.subSchema.items, viewPath, formViewPaths, formModelPaths, modelPathTranslations, dataTranslations, dataValidations);

    if (schema.messages )
        _processSchemaMessages.call(this, comp, schema.messages || (schema.subSchema && schema.subSchema.messages));

    if (schema.subscriptions || (schema.subSchema && schema.subSchema.subscriptions))
        _processSchemaSubscriptions.call(this, comp, schema.subscriptions || (schema.subSchema && schema.subSchema.subscriptions));
                ...
                case 'optional':
                    if (modelPath) {
                        ...
                        if (! notInModel) {
                            ...
                            _addDataValidation.call(this, validate, 'toModel', viewPath, schema);
                            _addDataValidation.call(this, validate, 'fromModel', modelPath, schema);
                        }
                    }
                    break;
                default:
    ...
    function _addDataValidation(validate, direction, path, schema) {
...
function processSubSchema(items) {
    items.forEach((comp, index) => {
        comp['compName'] = 'milo-' + index;
    })
}
