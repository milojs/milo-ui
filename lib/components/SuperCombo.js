...
function _changeSelected(direction) {
    ...
    const scrollTo = this._comboList.el.scrollTop + this._elementHeight * direction * 5;
    const validDirection1 = (relativePos > this._optionsHeight - this._elementHeight * 2 && direction === 1);
    const validDirection2 = (relativePos < this._elementHeight && direction === -1);

    if (validDirection1 || validDirection2) {
        if (typeof parent.scroll == 'function') {
            this._comboList.el.scroll(0, scrollTo);
        } else {
            this._comboList.el.scrollTop = scrollTo;
        }
    }
}
