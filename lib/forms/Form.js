    ...
    const inspector = hostObject && hostObject.inspector,
        ...
        onFormShown = function () {
            ...
            form.el.querySelectorAll('.form-tooltip').forEach(function (tooltip) {
                tooltip.addEventListener('mouseenter', initTooltipPosition);
                ...
                function initTooltipPosition() {
                    ...
                    const toolTipCentrePoint = tooltipbb.left + (tooltipbb.width / 2);
                    const wrapperCentrePoint = wrapbb.left + (wrapbb.width / 2);
                    wrapper.style.marginLeft = `${Math.max(0, toolTipCentrePoint - wrapperCentrePoint)}px`;

                    tooltip.addEventListener('mouseleave', function cleanup() {
                        wrapper.style.display = '';

                        tooltip.removeEventListener('mouseenter', initTooltipPosition);
